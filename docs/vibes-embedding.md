...
VIBES uses the hosted official Relay at `relay.paseo.sh:443`; its trust boundary and remaining service/remote-device gates live in [vibes-official-relay.md](vibes-official-relay.md). The Cloudflare Durable Object PoC is not part of the maintained source or Production route. Production Release is still held by official Relay admission, the unresolved browser Markdown resource-exhaustion advisory, and the exact Preview pointer retest; neither Stage authorizes a Production deployment.
