...
VIBES uses the hosted official Relay at `relay.paseo.sh:443`; its trust boundary and remaining service/remote-device gates live in [vibes-official-relay.md](vibes-official-relay.md). The Cloudflare Durable Object PoC is not part of the maintained source or Production route. Production Release is still held by official Relay admission and the exact Preview pointer retest; neither Stage authorizes a Production deployment.

The Page-aware successor closes the executable browser Markdown resource-exhaustion path without relying on an unavailable upstream fix. The app uses `markdown-it@14.3.1`, bounds each render to 64 KiB and 512 top-level nodes, disables automatic linkification and smartquotes, and injects that parser into every direct `react-native-markdown-display` entrypoint. Its nested legacy parser remains visible to package metadata but is never selected; a source coverage test fails if any renderer returns to the library default.
